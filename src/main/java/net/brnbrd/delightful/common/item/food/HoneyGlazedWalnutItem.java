...
public class HoneyGlazedWalnutItem extends DItem {
	public HoneyGlazedWalnutItem(Properties pProperties) {
		super(pProperties);
	}
	...
	public boolean isTag() {
		...
		return tags.isKnownTagName(DelightfulItemTags.NUTS_WALNUT) && !tags.getTag(DelightfulItemTags.NUTS_WALNUT).isEmpty();
	}
	...
	public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> comps, @NotNull TooltipFlag pIsAdvanced) {
		if (!this.isTag()) {
			...
			comps.add(Component.literal(DelightfulItemTags.NUTS_WALNUT.location().toString()).withStyle(ChatFormatting.UNDERLINE));
		} else {
