...
public class ChoppedCloverItem extends DItem {
	public ChoppedCloverItem(Properties pProperties) {
		super(pProperties);
	}
	...
	public boolean isTag() {
		...
		return tags.isKnownTagName(DelightfulItemTags.CLOVER) && !tags.getTag(DelightfulItemTags.CLOVER).isEmpty();
	}
	...
	public void appendHoverText(@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> comps, @NotNull TooltipFlag pIsAdvanced) {
		if (!this.isTag()) {
			...
			comps.add(Component.literal(DelightfulItemTags.CLOVER.location().toString()).withStyle(ChatFormatting.UNDERLINE));
		} else {
